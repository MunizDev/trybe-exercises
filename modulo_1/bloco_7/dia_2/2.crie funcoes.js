const lesson1 = {
  materia: "Matemática",
  numeroEstudantes: 20,
  professor: "Maria Clara",
  turno: "manhã",
};

const lesson2 = {
  materia: "História",
  numeroEstudantes: 20,
  professor: "Carlos",
};

const lesson3 = {
  materia: "Matemática",
  numeroEstudantes: 10,
  professor: "Maria Clara",
  turno: "noite",
};

// Crie uma função para adicionar o turno da manhã na lesson2. Essa função deve possuir três parâmetros, sendo eles: o objeto a ser modificado, a chave que deverá ser adicionada e o valor dela.
const addKeyValue = (objeto, newKey, newValue) => {
  objeto[newKey] = newValue;
};

addKeyValue(lesson2, "turno", "manhã");
console.log(lesson2);

// Crie uma função para listar as keys de um objeto. Essa função deve receber um objeto como parâmetro.
const listKeys = (objeto) => {
  const list = Object.keys(objeto);
  return list;
};
console.log(listKeys(lesson2));

// Crie uma função para mostrar o tamanho de um objeto.
const objLength = (objeto) => {
  const keyLength = Object.keys(objeto).length;
  return keyLength;
};
console.log(objLength(lesson2));

// Crie uma função para listar os valores de um objeto. Essa função deve receber um objeto como parâmetro.
const listValues = (objeto) => {
  const list = Object.values(objeto);
  return list;
};
console.log(listValues(lesson2));

// Crie um objeto de nome allLessons, que deve agrupar todas as aulas através do Object.assign. Cada chave desse novo objeto será uma aula, sendo essas chaves: lesson1, lesson2 e lesson3.
const allLessons = {};
allLessons.lesson1 = Object.assign({}, lesson1);
allLessons.lesson2 = Object.assign({}, lesson2);
allLessons.lesson3 = Object.assign({}, lesson3);
console.log(allLessons);

// Usando o objeto criado no exercício 5, crie uma função que retorne o número total de estudantes em todas as aulas.
const totalStudents = () => {
  let resultado = 0;
  resultado += allLessons.lesson1.numeroEstudantes;
  resultado += allLessons.lesson2.numeroEstudantes;
  resultado += allLessons.lesson3.numeroEstudantes;
  return resultado;
};
console.log(totalStudents());

// Crie uma função que obtenha o valor da chave de acordo com a sua posição no objeto. Por exemplo.
const getValueByNumber = (objeto, position) => {
  const keysObj = Object.keys(objeto);
  const keyChosen = keysObj[position];
  const value = objeto[keyChosen];
  return value;
};
console.log(getValueByNumber(lesson1, 0));

// Crie uma função que verifique se o par (chave / valor) existe na função. Essa função deve possuir três parâmetros, sendo eles: o objeto, o nome da chave e o valor da chave.
const verifyPair = (objeto, key, value) => {
  if (objeto[key] === value) {
    return true;
  } else {
    return false;
  }
};
console.log(verifyPair(lesson3, 'turno', 'noite'))
console.log(verifyPair(lesson3, 'materia', 'Maria Clara'))
